  ...
  var init = function(index, input, options) {
	...
	*/
	var Shape = function(id) {
		...
		this.hasBeenChanged = false;
			...
			Shape.prototype.setHasBeenChanged = function(status)
			{
			this.hasBeenChanged = status;
			}
			
			Shape.prototype.hasBeenChanged = function()
			{
	...
	var arrayFromPHP = <?php echo json_shapes; ?>;
	alert(arrayFromPHP);
	...
	$.each(allShapes, function(i) { 
		...
		if (thisShape.hasBeenChanged())
			{
			changedShapes.push(thisShape);
			}
	});
	alert("Updating " + changedShapes.length + " shapes.");
	$.post('./saveMapToDatabase.php', { jsonData: JSON.stringify(changedShapes), map_id: map_id}, function(response){
   alert(response);
	});
	};
    reset = function() {
	...
	*/
	...
	if (currentlyEditing)
		...
		thisShape.setHasBeenChanged(true);
