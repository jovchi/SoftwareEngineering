(function( $ ){
  ...
  var init = function(index, input, options) {
    ...
    var shapes, points, activePoint, settings, firstTime = true;
    var $reset, $areaViewer, $convertToImageMap, $clearCanvas, $clearCurrent, $canvas, ctx, image;
	...
	$convertToImageMap = $('<button type="button" class="btn"><i class="icon-trash"></i>Convert To ImageMap</button>');
	...
	$nextArea = $('<button id="nextButton" type="button" class="btn"><i class="icon-trash"></i>Next Area</button>');
    ...
    $(document).ready( function() {
      $(input).after('<br>', $canvas, '<br>', $reset, $clearCanvas, $nextArea, $clearCurrent, $convertToImageMap);
      $reset.click(reset);
	  $clearCanvas.click(clear);
	  $nextArea.click(nextArea);
	   $clearCurrent.click(clearCurrent);
	   $convertToImageMap.click(convertToImageMap);
	...
	//when you move a point
	  ...
	  if ((points.length == 8) && (firstTime == true))
	  {
	  console.log("Points length == 8");
	  points[4] = Math.round(e.offsetX);
	  
	  points[1] = Math.round(e.offsetY);
	  };
